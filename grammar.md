...
### 3. 注意事项
- 每次调用函数使用一个独立栈帧
- 栈帧弹栈弹空时，程序将尝试弹栈上一级栈帧，若最顶级栈帧（main栈帧）为空，则崩溃退出
- 函数调用结束时返回栈顶值压入父级栈帧并清空栈帧；此时若是空栈则不会返回
...
### 4. return操作
下面的例子里展示了return语法糖的使用，但这里被注释掉了（因为return语句可能是后期再实现）。当使用return时，函数将丢弃当前栈帧中除了栈顶元素之外的所有元素，并立即退出代码块。如果代码块结束时没有显式调用return，则默认返回栈顶元素的值，但如果栈为空则返回null。
```
{
  &a =
  1 a + ##return##
